...
export default function SignupPage() {
  ...
  const [formData, setFormData] = React.useState<{
    ...
    userName: string;
    phoneNumber: string;
    birthDate: Dayjs | null; // ISO 문자열로 변환
    gender: string;
    email: string;
    agreeTerms: string;
    agreePrivacy: string;
  ...
  const handleCheckDuplicate = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/api/applicants/check-id?loginId=${loginId}`
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:8080/api/applicants/signup',
        {
              ...
              control={
                <Checkbox
                  ...
                  onChange={e => {
                    ...
                    setHasError(prev => ({
                      ...prev,
                      agreeTerms: !isChecked, // 체크 안 하면 true (에러), 체크하면 false (정상)
                    }));
                  }}
                />
              }
              ...
              control={
                <Checkbox
                  ...
                  onChange={e => {
                    ...
                    setHasError(prev => ({
                      ...prev,
                      agreePrivacy: !isChecked, // 체크 안 하면 true (에러), 체크하면 false (정상)
                    }));
                  }}
                />
              }
