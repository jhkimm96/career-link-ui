...
export default function JobPostingManagementPage() {
        ...
        renderCell: params => {
          return (
            <Button
              ...
              variant="contained"
              onClick={() => {
                router.push(`/job-postings/edit?id=${params?.row.jobPostingId}`);
              }}
              disableElevation
              sx={{
                minWidth: 80,
  ...
  const fetchJobPostings = async () => {
    ...
    try {
      ...
      const res = await api.get('/emp/job-postings/manage', {
  ...
  const handleDeleteBulk = async () => {
    ...
    const ok = await confirm({
      title: '삭제 처리',
      message: `삭제 후 복구할 수 없습니다. 공고 [${selectionModel.ids.size}] 개를 일괄 삭제하시겠습니까?`,
      confirmText: '삭제',
      cancelText: '취소',
    });
    if (!ok) return;
    ...
    try {
      const res = await api.post('/emp/job-postings/delete-bulk', ids);
  ...
  const headerActions = (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
      <TextField
        size="small"
        placeholder="공고명·작성자·마감일자(YYYYMMDD) 검색"
        value={keyword}
